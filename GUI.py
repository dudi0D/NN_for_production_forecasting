...
class DialogApp(QWidget):
    def __init__(self):
        ...
        self.data = ''
        self.resize(450, 200)
        self.button1 = QPushButton('Load sample file')
        self.btnPlotOil = QPushButton("Annual oil production")
        self.btnPlotGas = QPushButton("Annual gas production")
        self.btnPredict = QPushButton('Forecast production parameters')
        self.btnPlotStats = QPushButton("&Sample parameters")
        ...
        choice_of_model_architecture.addItem('GRU Layer')
        choice_of_model_architecture.addItem('LSTM Layer')
        choice_of_model_architecture.addItem('8 GRU layers')
        choice_of_model_architecture.activated[str].connect(self.on_activated)
        label = QLabel("Choose of neural network architecture")
        label.setAlignment(QtCore.Qt.AlignCenter)
        layout.addWidget(label)
        layout.addWidget(choice_of_model_architecture, alignment=QtCore.Qt.AlignCenter)
        self.setWindowTitle('Production parameters forecasting')
        label1 = QLabel('Choose of forecasting parameter')
        ...
        choice_of_predicting_parameter.addItem('Обводненность, %')
        ...
        label_epochs = QLabel('Choose of epochs number')
        ...
        label2 = QLabel('Data preprocessing')
        data_preprocessing.addItem('Without additional preprocessing')
        data_preprocessing.addItem('Use sliding average')
    ...
    def plot_of_annual_gas(self):
        ...
        plt.title(self.file_name[52:].replace('.csv', ''))
        plt.xlabel('Years')
        plt.ylabel('Annual gas prod., mln. m3')
        plt.show()
    ...
    def predict_data(self):
        ...
        if self.current_architecture == 'LSTM Layer':
            model.add(LSTM(300))
        elif self.current_architecture == 'GRU Layer':
            model.add(GRU(300))
        else:
            for i in range(7):
